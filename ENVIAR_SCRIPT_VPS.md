# 📤 Como Enviar o Script para a VPS

O script `export-sqlite-vps.sh` precisa ser enviado para a VPS antes de executar.

## Opção 1: Enviar via SCP (Rápido)

Na sua máquina local (PowerShell ou Git Bash):

```bash
scp scripts/export-sqlite-vps.sh root@seu-vps-ip:/var/www/FinancialApps-def/scripts/
```

Substitua `seu-vps-ip` pelo IP da sua VPS.

## Opção 2: Criar Diretamente na VPS

Conecte na VPS e crie o arquivo:

```bash
ssh root@seu-vps-ip
cd /var/www/FinancialApps-def
mkdir -p scripts
nano scripts/export-sqlite-vps.sh
```

Depois cole o conteúdo do arquivo (veja abaixo).

## Opção 3: Git Commit/Push/Pull (Recomendado)

1. **Na sua máquina local:**
```bash
git add scripts/export-sqlite-vps.sh
git commit -m "feat: adiciona script de exportação SQLite para SQL Server"
git push
```
...
## Opção 4: Criar Manualmente na VPS (Se não tiver acesso Git)

Execute na VPS:

```bash
cat > /var/www/FinancialApps-def/scripts/export-sqlite-vps.sh << 'EOF'
#!/bin/bash
...
chmod +x /var/www/FinancialApps-def/scripts/export-sqlite-vps.sh
echo "✅ Script criado e configurado!"
```
