const sqlite3 = require('sqlite3').verbose();
const path = require('path');
const fs = require('fs');

// Encontrar o arquivo database.sqlite
let dbPath = path.join(process.cwd(), 'apps', 'api', 'database.sqlite');
if (!fs.existsSync(dbPath)) {
  dbPath = path.join(process.cwd(), 'database.sqlite');
}
if (!fs.existsSync(dbPath)) {
  console.error('❌ Arquivo database.sqlite não encontrado!');
  process.exit(1);
}

console.log(`📂 Usando banco de dados: ${dbPath}`);

const db = new sqlite3.Database(dbPath, (err) => {
  if (err) {
    console.error('❌ Erro ao conectar ao banco:', err.message);
    process.exit(1);
  }
  console.log('✅ Conectado ao banco de dados SQLite');
});

// Verificar se a coluna já existe
db.all("PRAGMA table_info(proposals)", (err, columns) => {
  if (err) {
    console.error('❌ Erro ao verificar colunas:', err.message);
    db.close();
    process.exit(1);
  }

  const columnNames = columns.map(col => col.name);
  console.log('📋 Colunas existentes na tabela proposals:', columnNames.join(', '));

  // Lista de colunas que precisam ser adicionadas
  const columnsToAdd = [
    { name: 'service_type', type: 'VARCHAR(100)' },
    { name: 'valor_proposta', type: 'DECIMAL(15,2)' },
    { name: 'valor_por_hora', type: 'DECIMAL(15,2)' },
    { name: 'horas_estimadas', type: 'VARCHAR(20)' },
    { name: 'previsao_conclusao', type: 'DATE' },
    { name: 'inicio_faturamento', type: 'DATE' },
    { name: 'vencimento', type: 'DATE' },
    { name: 'forma_faturamento', type: 'VARCHAR(50)' },
    { name: 'sistema_origem', type: 'VARCHAR(255)' },
    { name: 'sistema_destino', type: 'VARCHAR(255)' },
    { name: 'data_entrega_homologacao', type: 'DATE' },
    { name: 'data_entrega_producao', type: 'DATE' },
    { name: 'data_inicio_trabalho', type: 'DATE' },
    { name: 'data_faturamento', type: 'DATE' },
    { name: 'data_vencimento', type: 'DATE' },
  ];

  const columnsToAddFiltered = columnsToAdd.filter(col => !columnNames.includes(col.name));

  if (columnsToAddFiltered.length === 0) {
    console.log('✅ Todas as colunas já existem!');
    db.close();
    return;
  }

  console.log(`\n📝 Adicionando ${columnsToAddFiltered.length} coluna(s)...`);

  // Adicionar colunas uma por uma
  let completed = 0;
  columnsToAddFiltered.forEach((col, index) => {
    const sql = `ALTER TABLE proposals ADD COLUMN ${col.name} ${col.type}`;
    
    db.run(sql, (err) => {
      if (err) {
        console.error(`❌ Erro ao adicionar coluna ${col.name}:`, err.message);
      } else {
        console.log(`✅ Coluna ${col.name} adicionada com sucesso`);
      }
      
      completed++;
      if (completed === columnsToAddFiltered.length) {
        console.log('\n✅ Todas as colunas foram processadas!');
        db.close();
      }
    });
  });
});





