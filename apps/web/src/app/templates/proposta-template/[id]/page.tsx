  ...
  useEffect(() => {
    const loadTemplate = async () => {
      try {
        ...
        // Parsear conteúdo JSON se existir
        if (template.content) {
          try {
            const parsed = JSON.parse(template.content)
            
            if (parsed.serviceType) {
              setFormData(prev => ({ ...prev, serviceType: parsed.serviceType as ServiceType }))
            }
            ...
            if (parsed.fields && Array.isArray(parsed.fields)) {
              setSelectedFields(prev => {
                const newFields = prev.map(field => {
                  const savedField = parsed.fields.find((f: any) => f.key === field.key)
                  if (savedField) {
                    // Encontrar a ordem do campo salvo
                    const savedIndex = parsed.fields.indexOf(savedField)
                    return {
                      ...field,
                      selected: true,
                      order: savedIndex,
                ...
                // Ordenar pelos campos selecionados primeiro, depois pelos não selecionados
                const selected = newFields.filter(f => f.selected).sort((a, b) => a.order - b.order)
                const notSelected = newFields.filter(f => !f.selected).sort((a, b) => a.order - b.order)
                
                // Reordenar mantendo a ordem dos selecionados
                return [...selected, ...notSelected].map((field, index) => ({
                  ...field,
                  order: field.selected ? field.order : index,
                }))
              })
            }
          } catch (e) {
            console.error('Erro ao parsear conteúdo do template:', e)
          }
        }
      } catch (error: any) {
    ...
    <div className="min-h-screen bg-gray-100 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Editar Template de Negociação</h1>
        </div>
